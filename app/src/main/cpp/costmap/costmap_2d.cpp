...
// just initialize everything to NULL by default
    ...
    void Costmap2D::initMaps(unsigned int size_x, unsigned int size_y)
    {
        ...
        if (costmap_)
            delete[] costmap_;
        LOGI("Initialized the costmap Vector.");
    ...
    void Costmap2D::resetMap(unsigned int x0, unsigned int y0,
    ...
    bool
    Costmap2D::copyCostmapWindow(const Costmap2D &map, double win_origin_x,
                                 double win_origin_y,
                                 double win_size_x,
                                 double win_size_y)
    {
        ...
        if (!map.worldToMap(win_origin_x, win_origin_y, lower_left_x, lower_left_y)
            || !map.worldToMap(win_origin_x + win_size_x,
                               win_origin_y + win_size_y,
                               upper_right_x,
                               upper_right_y))
        {
        ...
        copyMapRegion(map.costmap_, lower_left_x, lower_left_y,
                      map.size_x_, costmap_, 0, 0,
                      size_x_, size_x_, size_y_);
        return true;
    }
    ...
    void Costmap2D::worldToMapEnforceBounds(double wx, double wy,
                                            int &mx, int &my) const
    {
    ...
    void Costmap2D::updateOrigin(double new_origin_x, double new_origin_y)
    {
        ...
        unsigned int cell_size_x =
                static_cast<unsigned int>(upper_right_x - lower_left_x);
        unsigned int cell_size_y =
                static_cast<unsigned int>(upper_right_y - lower_left_y);

        // we need a map to store the obstacles in the window temporarily
        unsigned char *local_map = new unsigned char[cell_size_x * cell_size_y];

        // copy the local window in the costmap to the local map
        copyMapRegion(costmap_,
                      static_cast<unsigned int>(lower_left_x),
                      static_cast<unsigned int>(lower_left_y),
        ...
        // now we want to copy the overlapping information back into the map,
        // but in its new location
        copyMapRegion(local_map, 0, 0, cell_size_x, costmap_,
                      static_cast<unsigned int>(start_x),
                      static_cast<unsigned int>(start_y), size_x_,
                      cell_size_x, cell_size_y);
    ...
    bool Costmap2D::setConvexPolygonCost(const base_info::bd_Polygon &polygon,
                                         unsigned char cost_value)
    {
        ...
        {
            MapLocation loc;
            if (!worldToMap(polygon[i].x(), polygon[i].y(),
                            loc.x, loc.y))
            {
                                        ...
                                        std::vector<MapLocation> &polygon_cells)
    {
        ...
        if (!polygon.empty())
        {
            unsigned int last_index = static_cast<unsigned int>(polygon.size() - 1);
