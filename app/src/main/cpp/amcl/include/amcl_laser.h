...
// Laseretric sensor model
    class AMCLLaser : public AMCLSensor
    {
        // Default constructor
    public:
        AMCLLaser(int max_beams, map_t *map);
    ...
    private:
        ...
        double beam_skip_distance;
        double beam_skip_threshold;
        //threshold for the ratio of invalid beams -
        // at which all beams are integrated to the likelihoods
        //this would be an error condition
        double beam_skip_error_threshold;

        //temp data that is kept before observations are integrated
        // to each particle (requried for beam skipping)
        int max_samples;
        int max_obs;
        double **temp_obs;
