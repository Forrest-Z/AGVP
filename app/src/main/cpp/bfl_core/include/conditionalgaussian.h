// $Id: conditionalgaussian.h 29890 2009-02-02 10:22:01Z tdelaet $
...
namespace bfl_core
{
    ...
    */
    class ConditionalGaussian
            : public ConditionalPdf<Eigen::Vector3d, Eigen::Vector3d>
    {
    public:
        /// Constructor
        /**
       ...
       arguments.
        */
        ConditionalGaussian(unsigned int dim = 0,
        ...
        virtual double ProbabilityGet(const Eigen::Vector3d &input) const;

        virtual bool SampleFrom(Sample <Eigen::Vector3d> &sample, int method = DEFAULT,
                                void *args = NULL) const;

        virtual bool SampleFrom(std::vector<Sample < Eigen::Vector3d> >& samples,
                                const unsigned int num_samples,
                                int method = DEFAULT, void *args = NULL) const;

    protected:
        // variables to avoid allocation on the heap during sampling
        mutable Eigen::Vector3d _diff;
        mutable Eigen::Vector3d _Mu;
        mutable Eigen::Matrix3d _Low_triangle;
        mutable Eigen::Vector3d _samples;
        mutable Eigen::Vector3d _SampleValue;
